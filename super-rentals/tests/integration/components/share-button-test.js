...
module('Integration | Component | share-button', function (hooks) {
  ...
  test('it supports passing @text argument', async function (assert) {
    await render(hbs`<ShareButton @text="foo">Hello Twitter</ShareButton>`)
    assert.equal(this.tweetParam('text'), 'foo')
  })

  test('it supports passing @hashtags argument', async function (assert) {
    await render(
      hbs`<ShareButton @hashtags="foo,bar,baz">Hello Twitter</ShareButton>`,
    )
    assert.equal(this.tweetParam('hashtags'), 'foo,bar,baz')
  })

  test('it supports passing @via argument', async function (assert) {
    await render(hbs`<ShareButton @via="foo">Hello Twitter</ShareButton>`)
    assert.equal(this.tweetParam('via'), 'foo')
  })
